...
it('can add the star name and star symbol properly', async() => {
    ...
    // 2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
    assert.equal(await instance.name(), 'Udacity Token');
...
it('lets a user transfer a star', async() => {
    ...
    let starId = 19;
    await instance.createStar('transferStar', starId, {from: owner1});

    await instance.transferStar(owner2, starId, {from: owner1});

    assert.equal(owner2,await instance.ownerOf(starId));
});
